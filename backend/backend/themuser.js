...
const insertUser = async () => {
    ...
    const hashedPassword2 = bcrypt.hashSync('editor123', 10);
    const hashedPassword3 = bcrypt.hashSync('guest123', 10);

    const sql = `
        INSERT INTO "Users" (username, password, firstname, lastname, "role_id", email, "createdAt", "updatedAt")
        VALUES
        ('adminUserNew', '${hashedPassword1}', 'Admin', 'User', 1, 'adminusernew@example.com', NOW(), NOW()),
        ('editorUserNew', '${hashedPassword2}', 'Editor', 'User', 2, 'editorusernew@example.com', NOW(), NOW()),
